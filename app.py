...
# ==========================================
    ...
    # --- MENU 2 (DI-UPGRADE: SEARCH, FILTER, & VALIDASI) ---
    elif menu == "📦 2. Cek Katalog Produk Toko":
        st.markdown('<h1>📋 Manajemen Katalog & Inventaris</h1>', unsafe_allow_html=True)
        
        # 1. Search Logic (Pencarian Berbasis Kriteria)
        search_query = st.text_input("🔍 Cari produk berdasarkan nama...", placeholder="Contoh: Susu, Beras...")
        
        # 2. Inventory & Performance Handling
        # Kita buat filter dinamis agar performa tetap cepat
        filtered_products = {
        ...
        if not filtered_products:
            st.warning("⚠️ Produk tidak ditemukan dalam database inventaris.")
        else:
            # 3. Data Presentation & Inventory Validation
            st.write(f"Menampilkan {len(filtered_products)} item dari total {len(st.session_state.database_produk)} produk.")
            
            for k, v in filtered_products.items():
                # 4. Validasi Keamanan & Status Stok
                # Simulasi pengecekan status (bisa dikembangkan ke logic stok di masa depan)
                ...
                <div style='display:flex; justify-content:space-between; align-items:center; padding:10px 0; border-bottom:1px solid #e2e8f0;'>
                    <div>
                        <div style='font-weight:700; color:#1e293b;'>📦 {k}</div>
                        <div style='font-size:11px; color:#64748b;'>Status: <span style='color:{color_stok}; font-weight:bold;'>{status_stok}</span></div>
                    </div>
                    <div style='text-align:right;'>
                        <div style='font-weight:800; color:#0f172a;'>Rp {v:,}</div>
                    </div>
                </div>
                """, unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)
